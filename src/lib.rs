...
impl Model {
    ...
    /// let b = rodeo.get_or_intern_static("b");
    /// let c = rodeo.get_or_intern_static("c");
    /// let d = rodeo.get_or_intern_static("d");
    ///
    /// let ab = VariableSet::new(&[a, b]);
    /// let ac = VariableSet::new(&[a, c]);
    /// let ad = VariableSet::new(&[a, d]);
    /// let bc = VariableSet::new(&[b, c]);
    /// let cd = VariableSet::new(&[c, d]);
    ///
    /// assert_eq!(
    ///     Model::new()
    ///     .add_relation(ab.clone())
    ///     .add_relation(bc.clone())
    ///     .find_loop_cores(),
    ///     Model::new()
    /// );
    ///
    ...
    ///     .add_relation(ab.clone())
    ///     .add_relation(bc.clone())
    ///     .add_relation(cd.clone())
    ///     .find_loop_cores(),
    ///     Model::new()
    /// );
    /// ```
    pub fn find_loop_cores(&self) -> Model {
        let mut reduced = self.clone();
        let mut unique = HashSet::new();
        let mut dups = HashSet::new();

        loop {
            for relation in reduced.relations.iter() {
                for variable in relation.0.iter().copied() {
                    if !unique.insert(variable) {
                        dups.insert(variable);
                    }
                }
            }

            unique.retain(|v| !dups.contains(v));
            if unique.is_empty() {
                break;
            }
            ...
            reduced =
                reduced
                    .relations
                    .into_iter()
                    .fold(Model::new(), |mut model, mut relation| {
                        relation.0.retain(|v| !unique.contains(&v));
    ...
    pub fn graph(&self) -> Vec<Vec<usize>> {
        ...
        for (i, relation) in self.relations.iter().enumerate() {
            // Find the earlier relations that this one shares at least one variable with.
            let mut edges = HashSet::new();
            for variable in relation.0.iter() {
                let seen = by_variable.entry(variable).or_insert_with(Vec::new);
                edges.extend(seen.iter().copied());
                seen.push(i);
            }

            // It isn't necessary to have edges in sorted order, but it's nice.
            let mut edges = Vec::from_iter(edges);
            edges.sort_unstable();

            // For each earlier relation this one is connected to, add the reverse edge too.
            /*
            for j in edges.iter().copied() {
                graph[j].push(i);
            }
            */
